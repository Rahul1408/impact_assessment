...
if [ "${key} " != " " ]; then
#cd ${directory}
fn0=`ssh ${user}@${server} 'grep -ilr '${key}' '${directory}' | grep -i \.sh$'`
fn1=`ssh ${user}@${server} 'grep -ilr '${key}' '${directory}' | grep -i \.nzsql$'`
#echo "$fn" > tmp08761
...
for f in $fn
do
echo `ssh ${user}@${server} ls -ltr --time-style=+"%Y-%m-%d~%H:%M:%S" $f` >> ${IAtooldir}/sam1
done
fi
...
fn2=`ssh ${user}@${server} 'find '${directory}' -iname "*'${key}'*"'`
else
fn2=`ssh ${user}@${server} 'find '${directory}' -iname "*'${key}'*".sh'`
fi
...
fn3=`ssh ${user}@${server} 'find '${directory}' -iname "*'${key}'*".nzsql'`
fi
...
for f2 in $fn4
do
echo `ssh ${user}@${server} ls -ltr --time-style=+"%Y-%m-%d~%H:%M:%S" $f2` >> ${IAtooldir}/sam1
done
